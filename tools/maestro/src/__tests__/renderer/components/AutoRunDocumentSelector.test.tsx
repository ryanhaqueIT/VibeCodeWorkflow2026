...
describe('AutoRunDocumentSelector', () => {
    ...
    const documentTree: DocTreeNode[] = [
      {
        ...
        children: [
          { name: 'nested-doc', type: 'file', path: 'folder1/nested-doc' },
          {
            ...
            children: [
              { name: 'deep-doc', type: 'file', path: 'folder1/subfolder/deep-doc' },
            ],
          },
        ],
      },
  ...
  describe('Form Submission', () => {
    ...
    it('shows Creating... state during submission', async () => {
      const slowCreateDocument = vi.fn().mockImplementation(
  ...
  describe('Folder Selection in Create Modal', () => {
    ...
    it('detects duplicate in subfolder', () => {
      ...
      expect(screen.getByText(/a document with this name already exists in folder1/i)).toBeInTheDocument();
    });
  ...
  describe('Helper Text', () => {
    it('shows extension hint when no folder selected', () => {
      ...
      expect(screen.getByText(/the \.md extension will be added automatically/i)).toBeInTheDocument();
    });
    ...
    it('shows path preview when folder is selected', () => {
      ...
      expect(screen.queryByText(/the \.md extension will be added automatically/i)).not.toBeInTheDocument();
