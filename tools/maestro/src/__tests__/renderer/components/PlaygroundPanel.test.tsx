...
vi.mock('../../../renderer/components/StandingOvationOverlay', () => ({
  StandingOvationOverlay: ({
    ...
    >
      Standing Ovation
      <button onClick={onClose} data-testid="close-ovation">Close</button>
    </div>
  ),
}));
...
vi.mock('../../../renderer/constants/conductorBadges', () => ({
  CONDUCTOR_BADGES: [
    { id: 'apprentice', level: 1, name: 'Apprentice', requiredTimeMs: 900000, shortName: 'Apprentice' },
    { id: 'assistant', level: 2, name: 'Assistant', requiredTimeMs: 3600000, shortName: 'Assistant' },
    { id: 'associate', level: 3, name: 'Associate', requiredTimeMs: 28800000, shortName: 'Associate' },
  ],
...
describe('PlaygroundPanel', () => {
  ...
  describe('Close Functionality', () => {
    ...
    it('unregisters layer on unmount', () => {
      const { unmount } = render(<PlaygroundPanel theme={mockTheme} themeMode="dark" onClose={mockOnClose} />);
    ...
    it('updates layer handler when onClose changes', () => {
      const { rerender } = render(<PlaygroundPanel theme={mockTheme} themeMode="dark" onClose={mockOnClose} />);
  ...
  describe('Confetti Tab - Origin Grid', () => {
    ...
    it('grid displays all 9 positions', () => {
      const gridButtons = screen.getAllByRole('button').filter(btn => {
        ...
        return text.includes('Top') || text.includes('Middle') || text.includes('Bottom') || text.includes('Center');
      });
  ...
  describe('Default Export', () => {
    it('default export is the same as named export', async () => {
      const { default: DefaultPlaygroundPanel } = await import('../../../renderer/components/PlaygroundPanel');
