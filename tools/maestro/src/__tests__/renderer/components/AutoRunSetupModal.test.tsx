...
describe('AutoRunSetupModal', () => {
  ...
  describe('rendering', () => {
    ...
    it('renders feature explanation section', async () => {
      ...
      expect(screen.getByText(/Auto Run lets you manage and execute Markdown documents/)).toBeInTheDocument();
    ...
    it('includes sessionName in placeholder when provided', async () => {
      ...
      expect(screen.getByPlaceholderText('Select Auto Run folder for My Agent')).toBeInTheDocument();
    });
  ...
  describe('edge cases', () => {
    ...
    it('cancels previous validation request when folder changes quickly', async () => {
      ...
      vi.mocked(window.maestro.autorun.listDocs).mockResolvedValue({ success: true, files: ['doc.md'] });
      ...
      await act(async () => { vi.advanceTimersByTime(100); }); // Less than debounce time

      // Second change - before first debounce completes
      fireEvent.change(input, { target: { value: '/second/path' } });
      await act(async () => { vi.advanceTimersByTime(300); }); // Now debounce fires
