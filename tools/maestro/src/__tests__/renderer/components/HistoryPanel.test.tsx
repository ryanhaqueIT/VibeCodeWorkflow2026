...
vi.mock('../../../renderer/components/HistoryDetailModal', () => ({
  HistoryDetailModal: ({ entry, onClose, onDelete, onNavigate, onUpdate, filteredEntries, currentIndex }: {
    entry: HistoryEntry;
    onClose: () => void;
    ...
    <div data-testid="history-detail-modal">
      ...
      {onDelete && (
        <button onClick={() => onDelete(entry.id)} data-testid="modal-delete">Delete</button>
      )}
      {onUpdate && (
        <button
...
vi.mock('../../../renderer/components/HistoryHelpModal', () => ({
  HistoryHelpModal: ({ onClose }: { onClose: () => void }) => (
    <div data-testid="history-help-modal">
      <button onClick={onClose} data-testid="help-modal-close">Close Help</button>
    </div>
  ),
}));
...
describe('HistoryPanel', () => {
  ...
  beforeEach(() => {
    ...
    (window as unknown as { maestro: { history: { getAll: typeof mockHistoryGetAll; delete: typeof mockHistoryDelete; update: typeof mockHistoryUpdate } } }).maestro.history = {
  ...
  describe('entry card rendering', () => {
    ...
    it('should render summary with truncation', async () => {
      const longSummary = 'This is a very long summary that should be truncated to three lines maximum in the history panel display because we do not want to show too much text.';
      const entry = createMockEntry({
        summary: longSummary,
      });
      mockHistoryGetAll.mockResolvedValue([entry]);
  ...
  describe('ActivityGraph component', () => {
    ...
    it('should display tooltip on bar hover', async () => {
      ...
      await waitFor(() => {
        ...
        const bars = container.querySelectorAll('[class*="flex-1"][class*="min-w-0"][class*="cursor-pointer"]');
  ...
  describe('session changes', () => {
    it('should reload history when session changes', async () => {
      ...
      mockHistoryGetAll.mockResolvedValue([
        createMockEntry({ summary: 'Entry from session 1' }),
      ]);
      ...
      // Change session
      mockHistoryGetAll.mockResolvedValue([
        createMockEntry({ summary: 'Entry from session 2' }),
      ]);
