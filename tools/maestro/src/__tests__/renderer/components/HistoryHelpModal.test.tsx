...
describe('HistoryHelpModal', () => {
  ...
  beforeEach(() => {
    ...
    mockGetCurrentStoragePath.mockResolvedValue('/Users/testuser/Library/Application Support/maestro');
  ...
  describe('Introduction Section', () => {
    it('renders the introduction text', () => {
      ...
      expect(screen.getByText(/The History panel tracks a synopsis of your work sessions/)).toBeInTheDocument();
    });
  ...
  describe('Entry Types Section', () => {
    ...
    it('renders the USER badge with correct styling', () => {
      ...
      expect(userBadge).toHaveClass('px-2', 'py-0.5', 'rounded-full', 'text-[10px]', 'font-bold', 'uppercase');
    });
    ...
    it('renders the AUTO badge with correct styling', () => {
      ...
      expect(autoBadge).toHaveClass('px-2', 'py-0.5', 'rounded-full', 'text-[10px]', 'font-bold', 'uppercase');
    });
    ...
    it('describes USER entry creation methods', () => {
      ...
      expect(screen.getByText(/Synopsis entries from your interactive work sessions/)).toBeInTheDocument();
    });

    it('describes AUTO entry generation', () => {
      render(<HistoryHelpModal {...defaultProps} />);

      expect(screen.getByText(/Entries automatically generated by the Auto Runner/)).toBeInTheDocument();
    });
  });
  ...
  describe('Viewing Details Section', () => {
    ...
    it('describes what detail view shows', () => {
      ...
      expect(screen.getByText(/Click any history entry to open the full details view/)).toBeInTheDocument();
    });
  ...
  describe('Resuming Sessions Section', () => {
    ...
    it('mentions Claude session ID preservation', () => {
      ...
      expect(screen.getByText(/Each history entry preserves the Claude session ID/)).toBeInTheDocument();
    });
  ...
  describe('Footer', () => {
    ...
    it('applies correct styling to "Got it" button', () => {
      ...
      expect(gotItButton).toHaveClass('px-4', 'py-2', 'rounded', 'text-sm', 'font-medium', 'transition-colors');
  ...
  describe('Dynamic History Path', () => {
    it('displays default storage path with home directory shortened', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('~/Library/Application Support/maestro/history/<sessionId>.json')).toBeInTheDocument();
      });
    });
    ...
    it('displays custom storage path when user has configured one', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('~/Dropbox/MaestroSync/history/<sessionId>.json')).toBeInTheDocument();
      });
    });
    ...
    it('displays Windows path with backslashes when detected', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText('~\\AppData\\Roaming\\maestro\\history\\<sessionId>.json')).toBeInTheDocument();
      });
    });
