...
describe('useLayerStack', () => {
  ...
  function createModalLayer(options: Partial<Omit<ModalLayer, 'id' | 'type'>> = {}): Omit<ModalLayer, 'id'> {
    return {
      type: 'modal',
  ...
  function createOverlayLayer(options: Partial<Omit<OverlayLayer, 'id' | 'type'>> = {}): Omit<OverlayLayer, 'id'> {
    return {
      type: 'overlay',
  ...
  describe('unregisterLayer', () => {
    ...
    it('should only remove the specified layer', () => {
      ...
      act(() => {
        id1 = result.current.registerLayer(createModalLayer({ priority: 100, ariaLabel: 'Layer 1' }));
        id2 = result.current.registerLayer(createModalLayer({ priority: 200, ariaLabel: 'Layer 2' }));
        id3 = result.current.registerLayer(createModalLayer({ priority: 300, ariaLabel: 'Layer 3' }));
      });
  ...
  describe('getTopLayer', () => {
    ...
    it('should update when layers change', () => {
      ...
      let highId: string;
      act(() => {
        highId = result.current.registerLayer(createModalLayer({ priority: 200, ariaLabel: 'High' }));
      });
  ...
  describe('debug API', () => {
    ...
    it('should preserve existing __MAESTRO_DEBUG__ properties', () => {
      ...
      expect((window.__MAESTRO_DEBUG__ as Record<string, unknown>)?.otherProperty).toBe('should remain');
      expect(window.__MAESTRO_DEBUG__?.layers).toBeDefined();
    });
