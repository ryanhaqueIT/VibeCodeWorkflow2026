...
describe('useTabCompletion', () => {
  ...
  describe('getSuggestions - git branches', () => {
    ...
    it('builds full value with prefix for branches', () => {
      ...
      const suggestions = result.current.getSuggestions('git checkout m');
      const branchSuggestion = suggestions.find(s => s.type === 'branch' && s.displayText === 'main');
  ...
  describe('getSuggestions - file completion', () => {
    ...
    it('only shows immediate children in path completion', () => {
      ...
      const fileAndFolderSuggestions = suggestions.filter(s => s.type === 'file' || s.type === 'folder');

      // Should have immediate children
      expect(fileAndFolderSuggestions.some(s => s.displayText.includes('components/'))).toBe(true);
      ...
      expect(fileAndFolderSuggestions.some(s => s.displayText.includes('Button.tsx'))).toBe(false);
    });
