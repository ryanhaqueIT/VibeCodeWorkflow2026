...
import { parseGitDiff, getFileName, getDiffStats, ParsedFileDiff } from '../../../renderer/utils/gitDiffParser';
  ...
  parseDiff: vi.fn((diffText: string) => {
    ...
    if (diffText.includes('@@')) {
      ...
      for (const match of hunkMatches) {
        ...
        const hunkContent = hunkEnd === -1
          ? diffText.slice(hunkStart)
          : diffText.slice(hunkStart, hunkEnd);
        ...
        hunks: [
          {
            content: '@@ -1,3 +1,4 @@',
            changes: [
              { type: 'insert', content: 'added', lineNumber: 1 },
            ]
          },
          {
            content: '@@ -10,3 +11,2 @@',
            changes: [
              { type: 'delete', content: 'removed', lineNumber: 10 },
            ]
          }
        ]
      }];
