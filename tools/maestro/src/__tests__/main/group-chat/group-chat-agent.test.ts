...
describe('group-chat-agent', () => {
  ...
  describe('addParticipant', () => {
    it('adds participant with new session', async () => {
      ...
      const participant = await addParticipant(chat.id, 'Client', 'claude-code', mockProcessManager);
    ...
    it('throws for non-existent chat', async () => {
      await expect(addParticipant('non-existent-id', 'Client', 'claude-code', mockProcessManager))
    ...
    it('throws for duplicate participant name', async () => {
      ...
      await expect(addParticipant(chat.id, 'Client', 'opencode', mockProcessManager))
    ...
    it('throws when spawn fails', async () => {
      ...
      await expect(addParticipant(chat.id, 'Client', 'claude-code', failingProcessManager))
    ...
    it('throws when moderator is not active', async () => {
      ...
      await expect(addParticipant(chat.id, 'Client', 'claude-code', mockProcessManager))
  ...
  describe('sendToParticipant', () => {
    ...
    it('logs message to chat log', async () => {
      ...
      expect(messages.some(m =>
        m.from === 'moderator->Client' &&
        m.content === 'Logged message'
      )).toBe(true);
    });
  });
  ...
  describe('sendToParticipant - errors', () => {
    it('throws for unknown participant', async () => {
      ...
      await expect(sendToParticipant(chat.id, 'Unknown', 'Hello', mockProcessManager))
        .rejects.toThrow(/not found/i);
    });

    it('throws for non-existent chat', async () => {
      await expect(sendToParticipant('non-existent-id', 'Client', 'Hello', mockProcessManager))
        .rejects.toThrow(/not found/i);
    });
    ...
    it('throws for inactive session', async () => {
      ...
      await expect(sendToParticipant(chat.id, 'Client', 'Hello', mockProcessManager))
  ...
  describe('removeParticipant', () => {
    ...
    it('throws for unknown participant', async () => {
      ...
      await expect(removeParticipant(chat.id, 'Unknown', mockProcessManager))
        .rejects.toThrow(/not found/i);
    });

    it('throws for non-existent chat', async () => {
      await expect(removeParticipant('non-existent-id', 'Client', mockProcessManager))
        .rejects.toThrow(/not found/i);
    });
