...
describe('group-chat-log', () => {
  ...
  beforeEach(async () => {
    ...
    testDir = path.join(os.tmpdir(), `group-chat-test-${Date.now()}-${Math.random().toString(36).slice(2)}`);
  ...
  describe('appendToLog', () => {
    ...
    it('uses ISO 8601 timestamp format', async () => {
      ...
      expect(new Date(timestamp).getTime()).toBeGreaterThanOrEqual(new Date(beforeTime).getTime() - 1000);
      expect(new Date(timestamp).getTime()).toBeLessThanOrEqual(new Date(afterTime).getTime() + 1000);
    });
  });
  ...
  describe('readLog', () => {
    ...
    it('preserves image references', async () => {
      ...
      await fs.writeFile(logPath, '2024-01-15T10:30:00.000Z|user|Check this [image:screenshot.png]\n');
      const messages = await readLog(logPath);
