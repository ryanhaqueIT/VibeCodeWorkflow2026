...
describe('Auto Run IPC Handlers', () => {
  ...
  describe('autorun:listDocs', () => {
    describe('successful operations', () => {
      ...
      it('should filter out non-markdown files', async () => {
        ...
        const entries = await mockReaddir('/test', { withFileTypes: true });
        const mdFiles = entries.filter((e: { name: string; isFile: () => boolean }) =>
      ...
      it('should return tree structure with nested folders', async () => {
        ...
        const tree = [
          ...
          },
          {
            ...
            path: 'Phase2',
            children: [
              { name: 'Task3', type: 'file', path: 'Phase2/Task3' },
            ],
          },
      ...
      it('should sort folders first, then files, both alphabetically', async () => {
        ...
        const sorted = [...entries].sort((a: { name: string; isDirectory: () => boolean }, b: { name: string; isDirectory: () => boolean }) => {
          if (a.isDirectory() && !b.isDirectory()) return -1;
          if (!a.isDirectory() && b.isDirectory()) return 1;
          return a.name.toLowerCase().localeCompare(b.name.toLowerCase());
        });

        expect(sorted.map((e: { name: string }) => e.name)).toEqual(['Alpha', 'Beta', 'alpha.md', 'zebra.md']);
      });
    });
    ...
    describe('error handling', () => {
      ...
      it('should return error for non-existent path', async () => {
        ...
        const result = { success: false, files: [], tree: [], error: 'ENOENT: no such file or directory' };
        expect(result.success).toBe(false);
    ...
    describe('path validation', () => {
      ...
      it('should validate resolved path is within folder', () => {
        ...
        const isValidPath = (relativePath: string): boolean => {
          const normalized = path.normalize(relativePath);
          if (normalized.includes('..') || path.isAbsolute(normalized) || !normalized.startsWith('images/')) {
            return false;
          }
  ...
  describe('autorun:deleteFolder', () => {
    ...
    describe('error handling', () => {
      ...
      it('should return error for rm failure', async () => {
        ...
        await expect(mockRm('/protected/Auto Run Docs', { recursive: true, force: true })).rejects.toThrow('EACCES');
      });
      ...
      it('should fail safety check for wrong folder name', () => {
        ...
        if (folderName !== 'Auto Run Docs') {
          const result = { success: false, error: 'Safety check failed: not an Auto Run Docs folder' };
  ...
  describe('Helper function: validatePathWithinFolder', () => {
    ...
    it('should return true for files within folder', () => {
      ...
      expect(validatePathWithinFolder('/test/autorun/subfolder/doc.md', '/test/autorun')).toBe(true);
    });
  ...
  describe('Integration: Full document lifecycle', () => {
    it('should support create -> read -> update -> delete workflow', async () => {
      ...
      expect(mockWriteFile).toHaveBeenLastCalledWith('/test/autorun/new.md', updatedContent, 'utf-8');
