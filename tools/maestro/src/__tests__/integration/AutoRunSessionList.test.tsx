...
const IntegrationTestWrapper = ({
  initialSessions = [
    createMockSession({ id: 'session-1', name: 'Session 1', autoRunContent: '# Session 1\n\n- [ ] Task 1' }),
    createMockSession({
      id: 'session-2',
      name: 'Session 2',
  ...
  const [showConfirmDialog, setShowConfirmDialog] = useState<{ message: string; onConfirm: () => void } | null>(null);
  ...
  return (
    ...
    <div style={{ display: 'flex', height: '100vh' }}>
      ...
      {activeSession && activeSession.autoRunFolderPath && (
        <div style={{ flex: 1 }} data-testid="autorun-container">
          <AutoRun
            ...
            onModeChange={(mode) => {
              setSessions(prev => prev.map(s =>
                s.id === activeSessionId ? { ...s, autoRunMode: mode } : s
              ));
            }}
            onOpenSetup={() => {}}
            onRefresh={() => {}}
      ...
      {activeSession && !activeSession.autoRunFolderPath && (
        <div data-testid="autorun-not-configured">
          Auto Run not configured for this session
        </div>
      )}
...
describe('Auto Run + Session List Integration', () => {
  ...
  describe('Session Selection Loads Correct Document', () => {
    it('clicking on a session in the list loads its Auto Run content', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('textbox')).toHaveValue('# Session 2\n\n- [ ] Task A\n- [ ] Task B');
      });
    });
        ...
        return (
          ...
          <div>
            ...
            <button
              ...
              onClick={() => setGroups(prev => prev.map(g => ({ ...g, collapsed: !g.collapsed })))}
            >
              Toggle Group
            </button>
            <p data-testid="group-state">
              Group collapsed: {groups[0].collapsed ? 'yes' : 'no'}
            </p>
            {activeSession && activeSession.autoRunFolderPath && (
              <AutoRun
                theme={theme}
  ...
  describe('Bookmarked Sessions and Auto Run', () => {
    ...
    it('selecting bookmarked session loads correct Auto Run content', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByRole('textbox')).toHaveValue('# Bookmarked Content\n\n- [ ] Starred Task');
      });
    });
  });
