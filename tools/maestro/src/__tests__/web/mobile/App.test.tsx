...
vi.mock('../../../web/mobile/SessionPillBar', () => {
  const MockSessionPillBar = ({ sessions, activeSessionId, onSelectSession, onOpenAllSessions, onOpenHistory }: {
    sessions: unknown[];
    ...
    return React.createElement('div', { 'data-testid': 'session-pill-bar' }, [
      ...
      onOpenAllSessions && React.createElement('button', {
        key: 'open-all-sessions',
        'data-testid': 'open-all-sessions',
        onClick: onOpenAllSessions,
      }, 'All Sessions'),
      onOpenHistory && React.createElement('button', {
        key: 'open-history',
...
vi.mock('../../../web/mobile/AllSessionsView', () => ({
  AllSessionsView: ({ sessions, activeSessionId, onSelectSession, onClose }: {
    sessions: unknown[];
    activeSessionId: string | null;
    onSelectSession: (id: string) => void;
    onClose: () => void;
    ...
    <div data-testid="all-sessions-view">
      <button data-testid="close-all-sessions" onClick={onClose}>Close</button>
      {sessions.map((s: any) => (
...
vi.mock('../../../web/mobile/MobileHistoryPanel', () => ({
  MobileHistoryPanel: ({
    ...
    <div data-testid="mobile-history-panel">
      <button data-testid="close-history" onClick={onClose}>Close</button>
...
vi.mock('../../../web/mobile/ResponseViewer', () => ({
  ResponseViewer: ({
    ...
    <div data-testid="response-viewer">
      <button data-testid="close-response-viewer" onClick={onClose}>Close</button>
      <button data-testid="navigate-prev" onClick={() => onNavigate(currentIndex - 1)}>Prev</button>
      <button data-testid="navigate-next" onClick={() => onNavigate(currentIndex + 1)}>Next</button>
...
vi.mock('../../../web/mobile/OfflineQueueBanner', () => ({
  OfflineQueueBanner: ({
    ...
    <div data-testid="offline-queue-banner">
      ...
      <button data-testid="clear-queue" onClick={onClearQueue}>Clear</button>
      <button data-testid="process-queue" onClick={onProcessQueue}>Process</button>
    </div>
  ),
}));
...
vi.mock('../../../web/mobile/TabBar', () => ({
  TabBar: ({
    ...
    <div data-testid="tab-bar">
      ...
      <button data-testid="new-tab" onClick={onNewTab}>New Tab</button>
      <button data-testid="close-tab" onClick={() => onCloseTab(activeTabId)}>Close Tab</button>
      <button data-testid="tab-search" onClick={onOpenTabSearch}>Search</button>
    </div>
  ),
}));
...
vi.mock('../../../web/mobile/TabSearchModal', () => ({
  TabSearchModal: ({
    ...
    <div data-testid="tab-search-modal">
      <button data-testid="close-tab-search" onClick={onClose}>Close</button>
    </div>
  ),
}));
...
describe('MobileApp', () => {
  ...
  describe('interrupt handling', () => {
    it('sends interrupt request via API', async () => {
      ...
      await act(async () => {
        mockHandlers.onSessionsUpdate?.([
          createMockSession({ id: 'session-1', state: 'busy' }),
        ]);
      });
  ...
  describe('tab management', () => {
    ...
    it('handles tabs changed event from desktop', async () => {
      ...
      await act(async () => {
        mockHandlers.onTabsChanged?.('session-1', [
          { id: 'tab-1', name: 'Tab 1', state: 'idle' },
          { id: 'tab-2', name: 'Tab 2', state: 'idle' },
        ], 'tab-2');
      });
  ...
  describe('offline queue', () => {
    it('displays offline queue banner when queue has items', async () => {
      mockQueue = [{ id: 'cmd-1', sessionId: 'session-1', command: 'test', mode: 'ai', timestamp: Date.now() }];
      mockQueueLength = 1;
