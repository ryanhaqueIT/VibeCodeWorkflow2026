...
describe('SlashCommandAutocomplete', () => {
  ...
  describe('Render conditions', () => {
    it('returns null when isOpen is false', () => {
      const { container } = render(
        <SlashCommandAutocomplete {...defaultProps} isOpen={false} />
      );
  ...
  describe('Styling', () => {
    it('has fixed max height based on isInputExpanded', () => {
      ...
      rerender(
        <SlashCommandAutocomplete {...defaultProps} isInputExpanded={true} />
      );
  ...
  describe('Edge cases', () => {
    it('handles empty commands array', () => {
      const { container } = render(
        <SlashCommandAutocomplete {...defaultProps} commands={[]} />
      );
      expect(container.firstChild).toBeNull();
    });
    ...
    it('handles commands with very long descriptions', () => {
      const longDescCommand: SlashCommand[] = [
        {
          command: '/long',
          description: 'This is a very long description that should still render properly without breaking the layout or causing any issues with the UI component rendering',
        },
      ];
    ...
    it('handles unicode in commands', () => {
      ...
      expect(
        screen.getByText('Command with emoji 🚀 and unicode 世界')
      ).toBeInTheDocument();
    });
  });
