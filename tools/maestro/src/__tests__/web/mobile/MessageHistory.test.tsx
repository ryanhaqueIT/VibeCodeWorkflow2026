...
vi.mock('lucide-react', () => ({
  ArrowDown: ({ style }: { style?: React.CSSProperties }) => (
    <span data-testid="arrow-down-icon" style={style}>↓</span>
  ),
}));
...
describe('MessageHistory', () => {
  ...
  describe('Message Rendering', () => {
    it('renders log entries with text content', () => {
      const logs: LogEntry[] = [
        createLogEntry({ text: 'Hello world', source: 'stdout' }),
      ];
    ...
    it('uses entry.id as key when available', () => {
      const logs: LogEntry[] = [
        createLogEntry({ id: 'unique-id-123', text: 'Test' }),
      ];
  ...
  describe('ANSI Code Stripping', () => {
    it('strips simple ANSI escape codes', () => {
      const logs: LogEntry[] = [
        createLogEntry({ text: '\x1b[31mRed text\x1b[0m' }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('handles text without ANSI codes', () => {
      const logs: LogEntry[] = [
        createLogEntry({ text: 'Plain text without codes' }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
  ...
  describe('Source Type Styling', () => {
    it('renders user messages with "You" label', () => {
      const logs: LogEntry[] = [
        createLogEntry({ source: 'user', text: 'User input' }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('renders stderr messages with "Error" label', () => {
      const logs: LogEntry[] = [
        createLogEntry({ source: 'stderr', text: 'Error output' }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('renders system messages with "System" label', () => {
      const logs: LogEntry[] = [
        createLogEntry({ source: 'system', text: 'System message' }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('renders stdout messages with "Output" label in terminal mode', () => {
      const logs: LogEntry[] = [
        createLogEntry({ source: 'stdout', text: 'Command output' }),
      ];
    ...
    it('falls back to stdout when source is undefined', () => {
      const logs: LogEntry[] = [
        createLogEntry({ source: undefined, text: 'Unknown source' }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
      expect(screen.getByText('AI')).toBeInTheDocument();
    });
    ...
    it('aligns user messages to the right', () => {
      const logs: LogEntry[] = [
        createLogEntry({ source: 'user', text: 'Right aligned' }),
      ];
      const { container } = render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('aligns non-user messages to the left', () => {
      const logs: LogEntry[] = [
        createLogEntry({ source: 'stdout', text: 'Left aligned' }),
      ];
      const { container } = render(<MessageHistory logs={logs} inputMode="ai" />);
      const messageDiv = container.querySelector('[style*="align-self"]');
  ...
  describe('Message Truncation', () => {
    ...
    it('does not truncate short text', () => {
      const logs: LogEntry[] = [
        createLogEntry({ text: 'Short text' }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('expands truncated message on click', () => {
      const logs: LogEntry[] = [
        createLogEntry({ id: 'expand-test', text: longText }),
      ];
      const { container } = render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('collapses expanded message on second click', () => {
      const logs: LogEntry[] = [
        createLogEntry({ id: 'collapse-test', text: longText }),
      ];
      const { container } = render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('truncates by lines when line count exceeds threshold', () => {
      const tenLines = 'Line 1\nLine 2\nLine 3\nLine 4\nLine 5\nLine 6\nLine 7\nLine 8\nLine 9\nLine 10';
      const logs: LogEntry[] = [
        createLogEntry({ text: tenLines }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('truncates by character count when under line threshold', () => {
      ...
      const logs: LogEntry[] = [
        createLogEntry({ text: longSingleLine }),
      ];
      render(<MessageHistory logs={logs} inputMode="ai" />);
    ...
    it('shows full text when expanded', () => {
      const logs: LogEntry[] = [
        createLogEntry({ id: 'full-text-test', text: longText }),
      ];
      const { container } = render(<MessageHistory logs={logs} inputMode="ai" />);
  ...
  describe('onMessageTap Callback', () => {
    ...
    it('does not crash when onMessageTap is not provided', () => {
      ...
      const { container } = render(
        <MessageHistory logs={logs} inputMode="ai" />
      );
  ...
  describe('maxHeight Prop', () => {
    ...
    it('uses flex layout when maxHeight is "none"', () => {
      ...
      expect(wrapper).toHaveStyle({ flex: '1', minHeight: '0', display: 'flex', flexDirection: 'column' });
    });
  });
  ...
  describe('Integration Scenarios', () => {
    it('handles complete conversation flow', () => {
      const logs: LogEntry[] = [
        ...
        createLogEntry({ source: 'stdout', text: 'Here is the function:\n```js\nfunction hello() {\n  console.log("Hello");\n}\n```', timestamp: 4000 }),
      ];
    ...
    it('handles terminal session conversation', () => {
      const logs: LogEntry[] = [
        ...
        createLogEntry({ source: 'stdout', text: 'total 24\ndrwxr-xr-x  5 user user 4096 Jan 15 10:00 .', timestamp: 2000 }),
