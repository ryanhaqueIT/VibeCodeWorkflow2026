...
export function DebugPackageModal({ theme, isOpen, onClose }: DebugPackageModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      window.maestro.debug.previewPackage()
        .then((preview) => {
    ...
    >
      ...
      >
        <p style={{ color: theme.colors.textMain }}>
          <strong>Privacy:</strong> This package does NOT include your conversations,
          API keys, or file contents. All paths are sanitized to remove usernames.
        </p>
      </div>
